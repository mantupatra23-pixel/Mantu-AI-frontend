...
export default function App() {
      ...
      try {
          const res = await fetch(`${BACKEND_URL}/api/save-project`, {
              ...
              headers: { 
                  ...
                  'Authorization': `Bearer ${localStorage.getItem('mantu_token')}` 
              },
              body: JSON.stringify({ 
  ...
  const toggleListening = (targetInput) => {
      ...
      recognition.onresult = (e) => {
          ...
          if (targetInput === 'followUp') {
              setFollowUpPrompt(initialPrompt + (initialPrompt ? ' ' : '') + trans);
          } else {
              setPrompt(initialPrompt + (initialPrompt ? ' ' : '') + trans);
          }
      };
  ...
  const triggerBuild = async (text, isFollowUp = false) => {
      ...
      newLogs.push({ 
          ...
          agent: 'MANTU OS', 
          status: 'Active', 
          details: isFollowUp ? 'Processing update request...' : 'Initializing Enterprise Engine...' 
      });
      ...
      try {
          const res = await fetch(`${BACKEND_URL}/api/build`, { 
              ...
              body: JSON.stringify({ 
                  prompt: text, 
                  existingFiles: isFollowUp ? generatedFiles : {} 
              }) 
          });
                  ...
                  if (part.startsWith('data: ')) {
                      try {
                          ...
                          if (data.type === 'log') {
                              ...
                              let cleanCode = String(data.code)
                                  .replace(/\\n/g, '\n')
                                  .replace(/\\"/g, '"')
                                  .replace(/```[a-zA-Z]*\n?/g, '')
                                  .replace(/```/g, '')
                              ...
                              setActionLogs(prev => [...prev, { 
                                  ...
                                  agent: 'SYSTEM', 
                                  status: 'Done', 
                                  details: 'NPM Files generated successfully!' 
                              }]);
                          }
                      } catch (err) {}
                  }
  ...
  const renderLivePreview = () => {
      ...
      const htmlKey = fileKeys.find(k => k.endsWith('index.html')) || fileKeys.find(k => k.endsWith('.html'));
      const cssKey = fileKeys.find(k => k.endsWith('styles.css') || k.endsWith('index.css') || k.endsWith('App.css'));
      const reactKey = fileKeys.find(k => k.endsWith('App.jsx') || k.endsWith('main.jsx') || k.endsWith('index.jsx'));

      let htmlFile = htmlKey ? generatedFiles[htmlKey] : `<div id="root" class="flex items-center justify-center h-screen font-sans bg-[#030303] text-gray-400">Wait, Building UI...</div>`;
      ...
      if (htmlFile.includes('</head>')) {
          htmlFile = htmlFile.replace('</head>', `${reactImports}</head>`);
      } else {
          htmlFile = `<head>${reactImports}</head>${htmlFile}`;
      }

      if (htmlFile.includes('</body>')) {
          htmlFile = htmlFile.replace('</body>', `${executeReact}</body>`);
      } else {
          htmlFile = `${htmlFile}${executeReact}`;
      }

      return `<!DOCTYPE html><html>${htmlFile}</html>`;
  };
  ...
  const handlePublish = async () => {
    ...
    if (publishMethod === 'aws' && (!awsTargetIp || !awsAuthKey)) {
        return alert("Please provide AWS IP and Auth Key.");
    }
    if (publishMethod === 'github' && (!githubToken || !repoName)) {
        return alert("Please provide GitHub Token and Repo Name.");
    }
    ...
    <div className="h-[100dvh] w-full flex flex-col font-sans overflow-hidden bg-[#050505] text-white relative">
      ...
      <div className="w-full bg-blue-900/20 border-b border-blue-900/50 text-blue-400 text-[10px] font-mono py-1.5 flex overflow-hidden whitespace-nowrap shrink-0 z-30">
           <div className="animate-marquee inline-block">
               🚀 User_92 pushed to GitHub in 2s... &nbsp; | &nbsp; ⚡ Mantu Native NPM Architecture Active... &nbsp; | &nbsp; 🧠 Llama-3 Enterprise Engine Active... &nbsp; | &nbsp; 🌍 Connected to AWS us-east-1...
           </div>
      </div>
      ...
      <nav className="h-14 flex items-center justify-between px-6 border-b border-[#1f1f23] bg-[#0a0a0c]/80 backdrop-blur-md shrink-0 z-20">
        ...
        <div className="flex items-center gap-4 text-sm font-medium">
           {view === 'home' && Object.keys(generatedFiles).length > 0 && (
               <button onClick={() => setView('editor')} className="text-xs font-bold text-white bg-blue-600 px-4 py-1.5 rounded-full hover:bg-blue-500 shadow-[0_0_15px_rgba(37,99,235,0.4)] mr-2">
                   Resume Editor →
               </button>
           )}
           {currentUser ? (
               <div className="flex items-center gap-4">
                   ...
                   <button onClick={handleLogout} className="text-xs text-red-400 hover:text-red-300 ml-2 font-bold">
                       Logout
                   </button>
               </div>
           ) : (
      ...
      {view === 'home' ? (
        <div className="flex-1 flex flex-col items-center pt-16 p-4 overflow-y-auto relative z-10 custom-scrollbar">
            ...
            <h1 className="text-5xl md:text-6xl font-extrabold mb-4 text-center tracking-tight">
                Build NPM Apps in <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-purple-500">Seconds</span>
            </h1>
            <p className="text-gray-400 mb-10 max-w-xl text-center text-sm md:text-base leading-relaxed">
                Describe your dream app. Mantu AI will architect the complete NPM workspace structure, bundle the project, and deploy it instantly.
            </p>
            ...
            <div className="w-full max-w-3xl bg-[#0d0d12]/80 backdrop-blur-xl border border-[#2b2b36] rounded-2xl flex flex-col shadow-2xl transition-all duration-300 focus-within:border-blue-500 focus-within:shadow-[0_0_30px_rgba(59,130,246,0.2)]">
                <textarea 
                    ...
                    placeholder="e.g. Build an AI video generator React Native app..." 
                ...
                <div className="flex items-center justify-between p-3 border-t border-[#1f1f23]">
                    ...
                    <button onClick={() => triggerBuild(prompt, false)} disabled={isGenerating} className="bg-blue-600 hover:bg-blue-500 text-white px-6 py-2 rounded-xl font-bold flex items-center gap-2 transition shadow-lg">
                    ...
                    <div className="w-full flex flex-col gap-3">
                        {projects.map((proj) => (
                            <button key={proj._id} onClick={() => loadProject(proj)} className="flex items-center gap-4 bg-[#0d0d12]/80 backdrop-blur-md border border-[#1f1f23] hover:border-blue-500/50 p-4 rounded-xl transition-all w-full text-left group">
                                <div className="bg-blue-500/20 text-blue-500 p-3 rounded-lg group-hover:scale-110 transition">
                                    <CodeIcon/>
                                </div>
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
            ...
            <div className="h-12 bg-[#0d0d12] border-b border-[#1f1f23] flex items-center justify-between px-4 shrink-0 overflow-x-auto custom-scrollbar">
                <div className="flex gap-1 bg-[#1a1a24] p-1 rounded-lg shrink-0">
                    <button onClick={()=>setActiveTab('preview')} className={`px-4 py-1 text-xs font-bold rounded flex items-center gap-2 ${activeTab === 'preview' ? 'bg-[#2b2b36] text-white shadow' : 'text-gray-400 hover:text-white transition'}`}>
                        <PlayIcon/> Live Preview
                    </button>
                    <button onClick={()=>setActiveTab('code')} className={`px-4 py-1 text-xs font-bold rounded flex items-center gap-2 ${activeTab === 'code' ? 'bg-[#2b2b36] text-white shadow' : 'text-gray-400 hover:text-white transition'}`}>
                        <CodeIcon/> View NPM Files
                    </button>
                </div>
                <div className="flex gap-2 shrink-0">
                    ...
                    <button onClick={() => setIsConsoleOpen(!isConsoleOpen)} className="px-3 py-1.5 text-xs font-bold bg-[#1a1a24] text-gray-300 hover:bg-[#2b2b36] rounded flex items-center gap-2 transition">
                        <TerminalIcon/> _Console
                    </button>
                    ...
                    <button onClick={() => setIsPublishModalOpen(true)} className="px-4 py-1.5 text-xs font-bold bg-blue-600 hover:bg-blue-500 text-white rounded flex items-center gap-2 shadow transition">
                        <CloudIcon/> Publish App
                    </button>
                </div>
            </div>
                        ...
                        {activeTab === 'code' ? (
                            ...
                            <div className="w-full h-full bg-white flex items-center justify-center">
                                {Object.keys(generatedFiles).length > 0 ? (
                                    ...
                                    <div className="text-gray-400 flex flex-col items-center gap-3"><SparkleIcon/> <span>No preview available.</span></div>
                                )}
                            </div>
                        )}
                    </div>
      ...
      {view === 'editor' && (
          <div className={`w-full transition-all duration-300 z-40 bg-[#0a0a0c] border-t border-[#1f1f23] shrink-0 ${isConsoleOpen ? 'h-56' : 'h-8'}`}>
              <div className="flex items-center justify-between px-4 h-8 cursor-pointer hover:bg-[#1a1a24]" onClick={() => setIsConsoleOpen(!isConsoleOpen)}>
                  <div className="text-[10px] font-bold text-gray-400 flex items-center gap-2 uppercase"><TerminalIcon/> OUTPUT CONSOLE</div>
                  ...
                  <div className="w-full md:w-1/3 bg-[#0a0a0c] border-r border-[#1f1f23] flex flex-col">
                      ...
                      <div className="p-2 flex flex-col gap-1 flex-1">
                          <button onClick={() => setPublishMethod('cloud')} className={`p-3 text-left rounded-lg transition ${publishMethod === 'cloud' ? 'bg-[#1a40af]/20 border border-blue-600 text-blue-500' : 'text-gray-400 hover:bg-[#1a1a24]'}`}>
                              <div className="font-bold text-xs flex items-center gap-2"><CloudIcon/> Upload to Netlify</div>
                              <div className="text-[10px] mt-1 opacity-70">Push raw NPM files</div>
                          </button>
                  ...
                  <div className="w-full md:w-2/3 bg-[#111116] p-6 flex flex-col relative">
                      ...
                      {publishMethod === 'cloud' && (
                          <div className="flex flex-col h-full mt-4 justify-center items-center text-center">
                              <h4 className="text-blue-500 font-bold text-lg mb-2">Upload to Netlify Cloud</h4>
                              <p className="text-gray-400 text-sm mb-8">This will upload your raw NPM structure. Connect your GitHub to Netlify later to trigger automated Vite builds.</p>
                              <button onClick={handlePublish} className="w-full max-w-xs bg-blue-600 hover:bg-blue-500 text-white py-3 rounded-lg font-bold text-sm transition flex justify-center items-center gap-2"><CloudIcon/> Push NPM Files</button>
                          </div>
                      )}
                      ...
                      {publishMethod === 'aws' && (
                          <div className="flex flex-col h-full mt-4">
                              ...
                              <div className="mb-6">
                                  <label className="text-xs text-gray-400 font-bold mb-1.5 block">Server Auth Key (.pem)</label>
                              ...
                              <button onClick={handlePublish} className="mt-auto w-full bg-orange-600 hover:bg-orange-500 text-white py-3 rounded-lg font-bold text-sm shadow-lg transition">Deploy NPM to AWS {(awsInstanceType || 'cpu').toUpperCase()}</button>
                          </div>
                      )}
                      ...
                      {publishMethod === 'github' && (
                          <div className="flex flex-col h-full mt-4">
                              ...
                              <button onClick={handlePublish} className="mt-auto w-full bg-gray-700 hover:bg-gray-600 text-white py-3 rounded-lg font-bold text-sm shadow-lg transition flex items-center justify-center gap-2"><GithubIcon/> Push NPM Code to GitHub</button>
                          </div>
                      )}
                  </div>
              </div>
                  ...
                  <div className="p-6 flex flex-col gap-4">
                      ...
                      {projectEnv.map((env, i) => (
                          <div key={i} className="flex gap-2 items-center">
                              <input type="text" value={env.key||""} onChange={(e) => { const n = [...projectEnv]; n[i].key = e.target.value; setProjectEnv(n); }} placeholder="API_KEY" className="w-1/3 bg-[#0A0A0E] border border-[#2b2b2b] rounded-lg p-2.5 text-sm text-white outline-none focus:border-yellow-500" />
                              <input type="text" value={env.value||""} onChange={(e) => { const n = [...projectEnv]; n[i].value = e.target.value; setProjectEnv(n); }} placeholder="Value" className="flex-1 bg-[#0A0A0E] border border-[#2b2b2b] rounded-lg p-2.5 text-sm text-white outline-none focus:border-yellow-500" />
                              <button onClick={() => setProjectEnv(projectEnv.filter((_, idx) => idx !== i))} className="text-red-500 font-bold px-2">X</button>
                          </div>
                      ))}
                      <button onClick={() => setProjectEnv([...projectEnv, { key: '', value: '' }])} className="text-xs text-blue-500 font-bold w-max">+ Add Variable</button>
                      <button onClick={() => setIsEnvModalOpen(false)} className="mt-4 w-full bg-blue-600 text-white py-3 rounded-lg font-bold text-sm">Save Keys</button>
                  </div>
              </div>
          </div>
      )}
